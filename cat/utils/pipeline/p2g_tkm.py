...
def pack_data(
    ...
    if is_S2P2G:
        max_len_text_label = max(len(''.join(x.split())) for x in text_labels) if isinstance(text_labels[0], str) else max(x.shape[0] for x in text_labels)
...
if __name__ == "__main__":
    ...
    if s_beg <= 2 and s_end >= 2:
        ...
        is_S2P2G = hyper_cfg["data"]["is_S2P2G"] if "is_S2P2G" in hyper_cfg["data"] else False
    ...
    if s_beg <= 4 and s_end >= 4:
        ...
        if "infer" in cfg_infr:
            ...
            elif intfname in ["cat.ctc.decode", "cat.rnnt.decode", "cat.ctc.decode_jsa", "cat.ctc.decode_p2g", "cat.ctc.decode_sample", "cat.aed.decode", "cat.aed.decode_mt5", "cat.aed.decode_mt5_tkm"]:
                if "input_scp" in infr_option:
                    ignore_field_data = True
            ...
            if ignore_field_data:
                ...
                for _set, scp in zip(testsets, f_scps):
                    ...
                    if intfname in ["cat.ctc.decode", "cat.rnnt.decode", "cat.ctc.decode_sample", "cat.aed.decode","cat.aed.decode_mt5", "cat.aed.decode_mt5_tkm"]:
